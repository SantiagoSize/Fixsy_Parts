...
import SupportPurchases from './components/SupportPurchases';
...
function InboxView() {
    ...
    const handleSendReply = (e: React.FormEvent) => {
        ...
        send({
            sender: user?.email || 'soporte@fixsy.cl',
        ...
        // Actualizar visualmente la selección actual
...
export default function SupportDashboard() {
    ...
    return (
        <div className="container-fluid py-4">
            ...
            {key === 'home' && (
                <div className="card mb-4" style={{ display: 'grid', justifyItems: 'center', gap: 8, padding: '2rem' }}>
                    {user?.profilePic ? (
                        <img className="fade-in" src={user.profilePic} alt="Foto de perfil" style={{ width: 96, height: 96, borderRadius: '50%', objectFit: 'cover' }} />
                    ) : (
                    ...
                    <p>Selecciona una opcion del menu izquierdo.</p>
                </div>
            )}

            {key === 'inbox' && <InboxView />}
            {key === 'tickets' && <Tickets />}
            {key === 'search' && <UserManagement />}
            {key === 'purchases' && <SupportPurchases />}
            {key === 'stock' && <ProductsList />}
            {key === 'profile' && <Profile />}
        </div>
    );
}
